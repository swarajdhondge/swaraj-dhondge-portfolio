...
export default function Skills() {
  return (
    <section id="skills" className="p-6 bg-gradient-to-br from-black via-gray-900 to-blue-900 text-white">
      <SectionTitle
        title="DevOps Skills"
        subtitle="Tools and technologies I use to build, automate, and scale systems"
      />
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6 justify-items-center">
        {skills.map((skill, index) => (
          <div
            key={index}
            className="flex flex-col items-center w-28 h-28 justify-center p-4 bg-gray-800 rounded-xl shadow-md hover:shadow-blue-600/30 transition transform hover:scale-105"
          >
            ...
            <p className="mt-2 text-sm text-blue-200 text-center">{skill.name}</p>
          </div>
        ))}
      </div>
    </section>
